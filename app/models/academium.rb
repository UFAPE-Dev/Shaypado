class Academium < ApplicationRecord
  ...
  validates :contato_comercial, presence: true, length: { is: 11 }, format: { with: /\A[0-9]+\z/, message: "Somente números" }
  validates :email_comercial, presence: true, length: {minimum: 5}, uniqueness: true, format: {with: /\A[\w+\-.]+@[a-z\d\-]+(\.[a-z]+)*\.[a-z]+\z/i}
